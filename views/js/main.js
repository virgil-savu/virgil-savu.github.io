...
var resizePizzas = function(size) {
  ...
  // Changed from querySelector to much faster getElementByID
...
for (var i = 2; i < 100; i++) {
  var pizzasDiv = document.getElementById("randomPizzas"); // moved outside the for loop
...
function logAverageFrame(times) {   // times is the array of User Timing measurements from updatePositions()
  ...
  //calculate fps
  //calculate each frame load
  var eachFrameLoad = 0;
  var fps = 0;

  eachFrameLoad = sum/10;
  fps = 1000/eachFrameLoad;

  console.log("Average time to generate last 10 frames: " + sum / 10 + "ms" + " - FPS:" + fps);
}
...
document.addEventListener('DOMContentLoaded', function() {
  var cols = 8;
  var s = 256;
  for (var i = 0; i < 32; i++) { // changed to i<32
